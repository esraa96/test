...
class CharityWebsite {
                ...
                if (entry.isIntersecting && !this.animatedCounters.has(entry.target)) {
                    ...
                    const timer = setInterval(() => {
                        ...
                        if (current >= target) {
                            ...
                            entry.target.textContent = this.formatNumber(Math.floor(current));
                        }
    ...
    setupStatsSlider() {
        ...
        // تعيين عرض السلايدر ليعرض 3 كاردات
        const cardWidth = 320; // w-80
        const cardMargin = 32; // mx-4 (16px each side)
        const totalCardWidth = cardWidth + cardMargin;
        
        // تحديد عدد الكاردات المعروضة حسب حجم الشاشة
        let cardsToShow = 3;
        if (window.innerWidth < 768) cardsToShow = 1;
        else if (window.innerWidth < 1200) cardsToShow = 2;
        
        const visibleWidth = totalCardWidth * cardsToShow;
        ...
        let timer;
        const totalSlides = 6;
        const maxPosition = totalSlides - cardsToShow; // آخر موضع يمكن الوصول إليه
        
        const move = (index) => {
            if (index < 0 || index > maxPosition) return;
            ...
            dots.forEach((dot, i) => {
                if (dot) {
                    dot.style.backgroundColor = i === index ? '#c1912c' : '#d1d5db';
        ...
        const auto = () => {
            ...
            timer = setInterval(() => {
                current = current >= maxPosition ? 0 : current + 1;
                move(current);
            }, 3000);
            this.timers.push(timer);
        };
        
        // إضافة event listeners للـ dots
        dots.forEach((dot, i) => {
            if (dot && i <= maxPosition) {
        ...
        // إيقاف/تشغيل السلايدر عند hover
...
// CSS for ripple effect, scroll progress, and slider
const rippleCSS = `
.btn {
    position: relative;
    overflow: hidden;
...
/* Stats Slider Styles */
.stats-slider {
    width: 1056px !important; /* 3 cards * 352px */
...
@media (max-width: 1200px) {
    .stats-slider {
        width: 704px !important; /* 2 cards * 352px */
    }
}

@media (max-width: 768px) {
    .stats-slider {
        width: 352px !important; /* 1 card * 352px */
    }
}
`;
